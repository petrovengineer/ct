    ...
    const buffer = await createReport({
      ...
      additionalJsContext:{
        ...
        getBarcode: (num)=>{
          ...
          return {width: 16, height: 3, data: buf, extension:'.png'}
        },
        formatTime
      },
      data
...
function createDublePasses(data, count){
  const template = './templates/pass-duble.docx';
  for(let i=1; i<=count; i=i+2){
    const output = `./docs/pass-${data.renter}-${i},${i+1}.docx`;
